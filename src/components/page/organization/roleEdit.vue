    ...
    <el-tab-pane label="基本信息" name="baseInfo">
      <el-form :model="model" ref="ruleForm" label-width="100px" v-show="tabActive =='baseInfo'">
        ...
        <el-form-item label="类型" prop="roleCategoryId" verify class="is-required">
          <t-dic-select dicType="base_rolecategory" v-model="model.roleCategoryId"></t-dic-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    onPermissionTreeCheck(item, node) {
      if (node.checkedKeys.includes(item.id)) {
        if (item.parentId != null && !node.checkedKeys.includes(item.parentId))
          node.checkedKeys.push(item.parentId);
          this.$refs.permissionTree.setCheckedKeys(node.checkedKeys);
      }
    },
    dataFormSubmit() {
      let self = this;

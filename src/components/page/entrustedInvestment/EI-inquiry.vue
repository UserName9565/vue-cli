...
<script>
import BfInquiry from './EI-inquiryWin/add';
 import EditForm from './EI-inquiryWin/edit';
export default {
  data() {
    return {
  ...
  methods: {
    doNew() {
      //this.$router.push("EI-addInquiry");
    ...
    doEdit(row) {
      //this.$router.push({ path: "/EI-editInquiry", query: { userId: row.id } });

...
<!-- 委投协议编号 -->
  <div>
    ...
    <el-form :inline="true" @keyup.enter.native="doSearch(1)">
      <el-row>
        ...
        <el-form-item>
          ...
          <el-button icon="el-icon-plus" type="primary" @click="doNew()">新增协议编号</el-button>
    ...
    <el-table :data="tableData3"  :header-cell-style="{background:'#e0f3ff',color:'#5f95b7'}" border stripe highlight-current-row="true" style="width: 100%">
      ...
      <el-table-column align="center"  prop="name" label="协议编号"></el-table-column>
      <el-table-column align="center"  prop="address" label="方案编号"></el-table-column>
      
      <el-table-column align="center"  prop="address" label="委托方"></el-table-column>
      <el-table-column align="center"  prop="address" label="协议金额(元)"></el-table-column>
    ...
    <div class="fontred" style="margin-top:10px">
        <p >*协议编号使用规则：，同一业务方案，多家委托方业务，会产品多个协议编号；同一委托方多笔投资业务，产生并采用同一个协议编号；</p>
        <p>*余额校验规则：协议编号生成前，需要做委托方账户余额校验操作。
</p>   
<p>【新增协议编号】中，余额校验，仅查询委托方账户可用余额是否满足投资条件，不做资金冻结；</p>
<p>协议编号审批中，余额校验，在审批通过后，会同时查询委托方账户可用余额是否满足投资条件，并做资金冻结业务</p>
      </div>
    </el-card>
...
<script>
import EditForm from "./EI-protocolNumberWin/edit";
